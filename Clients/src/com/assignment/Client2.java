...
/**
 * The class is responsible for Client 2 operations.
 * 
 * @author Tanveer
 *
 */
public class Client2 extends JFrame {
	...
	private static Client2 frame = null;
	 ...
	 */
	private Client2(String id, int time) throws UnknownHostException, IOException {

		logicalTime = time;

		String[] users = { "User 1", "User 3" };
		...
		Runnable sendMessage = () -> {
			try {
				while (true) {
					...
					Message message = new Message("User 2", users[random.nextInt(2)], Integer.toString(logicalTime));
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Message message = new Message("User 2", null, "REQUEST_TO_CONNECT");
				try {
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Message message = new Message("User 2", null, "REQUEST_TO_DISCONNECT");
				try {
					HttpEncoder.encode(bufferedWriter, message);
					t1.interrupt();
					t2.interrupt();
		...
		scrollPane.setBounds(29, 149, 559, 314);
